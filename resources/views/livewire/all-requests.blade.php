<div>
            ...
            <div class="mx-auto justify-center items-center w-[20%] bg-gray-300 rounded-md">
                ...
                <form wire:submit="save()" class="flex rounded-sm flex-col w-full p-3 bg-gray-100">
                    ...
                    <div class="flex justify-center">
                        <button wire:click="createSprintAndTasks($selectedRows)" type="submit"
    ...
    {{-- <table class="table-auto bg-yellow-200 rounded-md ">
            <thead class="tracking-wide ">
                ...
                <tr class="{{ !in_array($request->id, $selectedRows) ? 'bg-white border-b' : 'border-b bg-gray-300' }}">
                    <td class="p-4">{{ $request->requesterId }}</td>
                    <td class="p-4"><a class="underline italic hover:text-gray-500 duration-300" href="/projects/view/{{ $request->id }}">{{ $request->title }}</a></td>
                    ...
                    <td class="p-4">
                        ...
                        <button wire:click="toggleSelect({{ $request->id }})" title="Select" class="bg-green-600 rounded-full p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-200">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                            </svg>
                        </button>
                        @else
                        <button wire:click="toggleSelect({{ $request->id }})" title="Cancel" class="bg-orange-400 p-2 rounded-full"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-100">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                            </svg>
                        </button>
                        @endif
                        <button wire:click="rejectRequest({{ $request->id }})" title="Reject Request" class="bg-red-600 rounded-full p-2"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-gray-200">
