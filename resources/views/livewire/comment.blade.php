...
<div class="flex flex-col mx-auto items-center w-full">
    ...
    <div class="bg-gray-100 rounded-lg p-4 w-full">
        ...
        @if ($mainComments->count() == 0)
            <div class="mt-2 space-y-2 mt-[10%]">
                <hr class="border-2 border-gray-400 mb-[5%]">
                <p class="text-center text-3xl italic">No Comments Provided Yet</p>
            </div>
        @else
            <h2 class="text-4xl text-center text-gray-800 font-bold mb-2">Comments</h2>
            @if ($openLikersForm == true)
                <div
                    class="w-[20%] fixed left-[40%] bottom-[40%] z-10 bg-gray-100 p-2 border-2 border-blue-400 rounded-md ">
            ...
            @foreach ($mainComments as $mainComment)
                <div class="mt-[1%] space-y-2">
                    {{-- <p>{{dd($mainComments)}}</p> --}}
                    <!-- Individual Comments (replace with your actual comment data) -->
                    @if ($mainComment->commentable_id == $currentTask->id && $mainComment->c2Id == null)
                        <div class="flex flex-col bg-white rounded-md p-2 shadow-md">
                            <div class="flex items-center justify-between mb-1">
                                <div class="flex items-center">
                                    <p class="text-lg text-gray-800 font-semibold">{{ $mainComment->commenterName }}</p>
                                </div>
                                @foreach ($likesDislikes as $elem)
                                    ...
                                    @if ($elem['commentId'] == $mainComment->id && $mainComment->main_comment == null)
                                        <div class="flex items-center">
                                            <span class="cursor-pointer" wire:click="showLikers({{ $mainComment->id }})"
                                                class="text-xl">{{ $elem['likes'] }}</span>
                                            <button wire:click="likeComment({{ $mainComment->id }})"
                            ...
                            <div class="flex justify-end">
                                {{-- !in_array($mainComment->id, $isReplyFormOpened) --}}
                            ...
                            <button wire:click="getCommentReplies({{ $mainComment->id }})"
                                class="w-fit text-start text-lg font-semibold hover:text-gray-600 duration-300">
                                <span>
                                    @if (!in_array($mainComment->id, $showRepliedComments))
                                        Show Replies <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                            class="w-7 pb-1 h-7 inline">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M19.5 5.25l-7.5 7.5-7.5-7.5m15 6l-7.5 7.5-7.5-7.5" />
                                        </svg>
                                    @elseif(in_array($mainComment->id, $showRepliedComments))
                                        Close Replies <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                            class="w-7 pb-1 h-7 inline">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <div class="replied-comments mt-4">
                        ...
                        @if (in_array($mainComment->id, $showRepliedComments))
                            @foreach ($replies as $repliedComment)
                                {{-- <p>{{dd($replies)}}</p> --}}
                                {{-- <p>{{dd($replies)}}</p> --}}
                                @if ($repliedComment->commentable_id == $currentTask->id)
